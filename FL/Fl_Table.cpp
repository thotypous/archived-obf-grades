...
void Fl_Table::row_height(int row, int height) {
    ...
    if ( row < (int)_rowheights.size() && _rowheights[row] == height ) {
        return;
    }		// OPTIMIZATION: no change? avoid redraw
...
void Fl_Table::recalc_dimensions() {
    ...
    // Make scroll bars disappear if window large enough
    {
        ...
        if ( !hideh & hidev ) {
            hidev = (( table_h - tih + SCROLLBAR_SIZE ) <= 0 );
        }
        if ( !hidev & hideh ) {
            hideh = (( table_w - tiw + SCROLLBAR_SIZE ) <= 0 );
        }
...
void Fl_Table::rows(int val) {
    ...
    _rows = val;
    {
        ...
        while ( now_size < val ) {
            _rowheights[now_size++] = default_h;
        }	// fill new
    }
    table_resized();

    // OPTIMIZATION: redraw only if change is visible.
    if ( val >= oldrows && oldrows > botrow ) { /* NO REDRAW */
    } else {
        redraw();
    }
}
